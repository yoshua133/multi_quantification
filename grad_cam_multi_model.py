...
from pytorch_grad_cam import GuidedBackpropReLUModel as GuidedBackpropReLUModel2
from pytorch_grad_cam.utils.model_targets import ClassifierOutputTarget
from pytorch_grad_cam.utils.image import show_cam_on_image as show_cam2
...
def write_output(image_path, output_path, grad_cam, model, use_cuda, target_category,gb_model):
    ...
    #embed()
    print("write output to " + output_path + "/a"+str(target_category+0))
...
#replace this
def write_output2(image_path, output_path, grad_cam,  target_category, gb_model=None):
    targets = []
    for i in target_category:
        targets.append(ClassifierOutputTarget(i))
    img = cv2.imread(image_path)#, 1)
    ...
    input_img = torch.tensor(img_c).unsqueeze(0)#preprocess_image(img)
    #embed()
    grayscale_cam = grad_cam(input_tensor=input_img, targets=targets)
    grayscale_cam = grayscale_cam[0, :]
    visualization = show_cam2(rgb_img, grayscale_cam, use_rgb=True)

    #grayscale_cam = cv2.resize(grayscale_cam, (img.shape[1], img.shape[0]))
    #embed()
    cv2.imwrite(os.path.join(output_path, "a"+str(target_category)+"_"+"visualization.jpg"), visualization)
...
if __name__ == '__main__':
    ...
    model_name = 'resnext50_32x4d'  #只能做resnet或者resnext
    model_size = '201'
    crop_method = 1  #0代表整张x光范围, 1代表上牙齿范围 跟训练时候的设定一样
    need_attributes_idx = [4,5,6,7,8,9,10,11,12] #跟训练时候的设定一样
    start_patient_id = 680  #开始画可视化图的病人编号
    end_patient_id = 1035    #结束的标号
    model_pkl_name = "20231128_163406part1attri_4_12resnext50_32x4d_201_1"  #在train_file下找到需要可视化的模型的文件名，黏贴过来,需要跟路径中间的模型名称完全一样
    target_catogory = [6] #target category表示对need_attributes_idx中的第几个index计算cam,index是从0开始的, 如果 need_attributes_idx = [4,5,6], target = [0,1]就对应[4,5]
    #如果target_catogory中指标多于1个的话 只有一张图，不然是三张图。建议每个指标分别可视化。
    pretrain = False
    ...
    test_id =0 
    img_root = "E://ai_images_1_1036(important)//"
    visual_dir = img_root + model_pkl_name #"E://20231128_163022part1attri_4_12resnext50_32x4//"
    if crop_method==1:
    ...
    #load_model_path_i = os.path.join(save_dir ,'20231117_094033part1attri_4_12resnext50_32x4d_201_1','model_param.pkl')
#'20210418_164709part0_apr18_revised_crop_1_725_train_aug_only_2_p_0.2_78resnext101_32x8d_101pretrain-False'
    ...
    #embed()
    #grad_cam = GradCam(model=model, feature_module=model.layer4, target_layer_names=["2"], use_cuda=use_cuda)
                       
                                           
    #this place
    target_layers = [model.layer4[-1]]
    grad_cam = GradCAM(model=model, target_layers=target_layers, use_cuda= use_cuda)#可以把这里的gradcam替换成17行的其他模型
    ...
    gb_model = GuidedBackpropReLUModel2(model=model, use_cuda=use_cuda)
    #target_category = 1
    ...
    for patient_id in range(start_patient_id,end_patient_id):
        ...
        for tooth_id in ['11','12','21','22']:
            ...
            for file_name in os.listdir(img_dir):
                ...
                #        break 
                write_output2(image_path, output_path, grad_cam, target_catogory, gb_model)
                #for target_catogory_i in target_catogory:
                #    write_output(image_path, output_path, grad_cam, model, use_cuda, target_catogory_i,gb_model)
