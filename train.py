...
if __name__ ==  '__main__':
    ...
    from config import BATCH_SIZE, PROPOSAL_NUM, SAVE_FREQ, LR, WD, resume, save_dir,use_attribute, file_dir, max_epoch, need_attributes_idx,use_uniform_mean,anno_csv_path, \
    ...
    os.environ['CUDA_VISIBLE_DEVICES'] = '0'#指定用的显卡
    ...
    if model_name == 'resnet':
        if model_size == '50':
            net = resnet.resnet50(pretrained=pretrain)           
        elif model_size == '34':
            net = resnet.resnet34(pretrained=pretrain)
        elif model_size == '101':
            net = resnet.resnet101(pretrained=pretrain)
        elif model_size == '152':
            net = resnet.resnet152(pretrained=pretrain)        
    elif model_name == 'vgg':
        if model_size == '11':
            net = torchvision.models.vgg11_bn(pretrained=pretrain)
        elif model_size == '16':
            net = torchvision.models.vgg16_bn(pretrained=pretrain)
        elif model_size == '16_nobn':
            net = torchvision.models.vgg16(pretrained=pretrain)
        elif model_size == '19':
            net = torchvision.models.vgg19_bn(pretrained=pretrain)
            
    elif model_name == "resnext101_32x8d":
        net = torchvision.models.resnext101_32x8d(pretrained=pretrain)
    
    elif model_name == "resnext50_32x4d":
        net = torchvision.models.resnext50_32x4d(pretrained=pretrain)
    
    elif model_name == "inception_v3":
        net = torchvision.models.inception_v3(pretrained=pretrain, aux_logits =False )
        
    elif model_name == "wide_resnet101_2":
        net = torchvision.models.wide_resnet101_2(pretrained=pretrain)
    elif  model_name == "wide_resnet50_2":
        net = torchvision.models.wide_resnet50_2(pretrained=pretrain)
        ...
        if model_size == '121':
            ...
            net = torchvision.models.densenet161(pretrained=pretrain)  
        elif model_size == '169':
            net = torchvision.models.densenet169(pretrained=pretrain)  
        elif model_size == '201':
            net = torchvision.models.densenet201(pretrained=pretrain)   
    #修改最后一层fc
    if model_name == "densenet":
        in_feature = net.classifier.in_features
        net.classifier = torch.nn.Linear(in_feature, num_of_need_attri)
    else:
        in_feature = net.fc.in_features
        net.fc = torch.nn.Linear(in_feature, num_of_need_attri)
    
    print("use model",model_name, model_size) 
    ...
    if loss_name == "L1":   #对难样本不是特别敏感
        creterion = torch.nn.L1Loss(reduction= 'none')
    elif loss_name == "L2":  #对难样本最敏感
        creterion = torch.nn.MSELoss(reduction= 'none')
    elif loss_name == "smooth_L1":   #对难样本最不敏感
        creterion = torch.nn.SmoothL1Loss(reduction= 'none', beta = loss_alpha)
    elif loss_name == "huber":
        creterion = torch.nn.HuberLoss(reduction= 'none', delta = loss_alpha)   
    
    
    # read dataset
    print("use anno csv:",anno_csv_path)
    #embed()
    ...
    valset = dataset.tooth_dataset_val(anno_path=anno_csv_path,test_id = test_id)
    valset.attributes_mean = trainset.attributes_mean
    valset.attributes_std = trainset.attributes_std
    ...
    valloader = torch.utils.data.DataLoader(valset, pin_memory= False)
    # define model
    ...
    head=['train_loss_unit_degree','train_ori_loss_unit_std','test_loss','test_ori_loss','target_loss','val_loss', 'val_ori_loss']
    ...
    save_csv_path_val = os.path.join(file_dir,'val_dataset_{}.csv'.format(test_id))#,test_save_name))
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        for i, data in enumerate(trainloader):
            ...
            loss_delta = torch.abs(output - target)#.reshape(-1) # cross entrophy
            weight = torch.ones(loss.shape).cuda()
        ...
        if epoch % 5 == 0 or epoch==1:
            ...
            for i, data in enumerate(valloader):
                with torch.no_grad():
                    ...
                    batch_size = img.size(0)
                    #print('val batch size',batch_size)#bs=1
                    val_num += batch_size
                    raw_optimizer.zero_grad()
                    start = time.time()
                    output = net(img)
                    ...
                    unnorm_tar = target.cpu().numpy() * (trainset.attributes_std[use_uniform_mean]).reshape(-1)
                    if unnorm_delta.shape[-1]==1:
                        val_target_loss += unnorm_delta.sum()
                    elif unnorm_delta.shape[-1]==3:
                        val_target_loss += unnorm_delta[:,-1].sum()
                    elif unnorm_delta.shape[-1]==2:
                        val_target_loss += np.abs( ( (unnorm_out[:,0]-unnorm_out[:,1]) - (unnorm_tar[:,0]-unnorm_tar[:,1]) ) ).sum()
                    else:
                        val_target_loss += unnorm_delta.sum()
                        
                    # from val_dataset.py
                    target_unnorm = (target.cpu().numpy()* valset.attributes_std[use_uniform_mean])+valset.attributes_mean[use_uniform_mean]
                    output_unnorm = (output.cpu().numpy()* valset.attributes_std[use_uniform_mean])+valset.attributes_mean[use_uniform_mean]
                    target_unnorm = target_unnorm.reshape(-1)
                    output_unnorm = output_unnorm.reshape(-1)
                    cur_row =[]
                    cur_row.append(str(cur_use_attri[0]))#.item()))                                                                                                 
                    ...
                    if use_9:
                        ...
                        unnorm_delta = ori_delta * valset.attributes_std[use_uniform_mean]
                    mae_total = mae_total + unnorm_delta
                    mse_total = mse_total + unnorm_delta.reshape(-1) * unnorm_delta.reshape(-1)
                    ...
                    """
                    #loss is the mean distance between two tensor
                    val_loss += unnorm_delta.mean()*batch_size
                    val_ori_loss += ori_delta.mean()*batch_size
                    # calculate accuracy
            output_csv.insert(0,[str(total_time),str(val_num),str(total_time/val_num)])
            mae_print = list()
            if use_9:
                mae_print.append(str(mae_total/val_num))
            else:
                mae_print.append(" mae ")
                for i in range(mae_total.shape[0]):
                    mae_print.append(str(mae_total[i]/val_num))
                mae_print.append(" mse ")
                for i in range(mse_total.shape[0]):
                    mae_print.append(str(mse_total[i]/val_num))
            output_csv.insert(0,mae_print)
            output_csv.insert(0,["0~1",str(seg_dict[1]/val_num)])
            output_csv.insert(0,["1~2.5",str(seg_dict[2]/val_num)])
            output_csv.insert(0,["2.5~5",str(seg_dict[5]/val_num)])
            output_csv.insert(0,["5~10",str(seg_dict[10]/val_num)])
            #embed()
            loss_csv=pd.DataFrame(columns=test_head,data=output_csv)
            #embed()
            loss_csv.to_csv(save_csv_path_val,encoding='gbk')
            ...
            average_loss.append([train_loss/train_num, train_ori_loss/train_num, test_loss/test_num, test_ori_loss/test_num, test_target_loss/test_num,val_loss/val_num, val_ori_loss/val_num])
